...
try{
    ...
    if(empty($checkingDiaryExistResult)==true) { 
        ...
        if(empty($monthCheckResult)==true) { 
            throw new exception('B 지정한 월의 저장된 날짜 데이터가 없습니다.', 409);
        }
        else {  
            ...
            if(empty($weekCheckResult)==true){
                ...
                while($weekRecord = mysqli_fetch_assoc($weekCheckResult)){
                    ...
                    $DistinctColorArr=array_count_values($colorArr);
                    foreach($DistinctColorArr as $key=>$value){
                        $colorData = json_encode(['x' => $key, 'value' => $value]);
                        echo $colorData."\n";

                    }
                    echo "\n";
                    $DistinctkeywordArr=array_count_values($keywordArr);
                    foreach($DistinctkeywordArr as $key=>$value){
                        $keywordData = json_encode(['x' => $key, 'value' => $value]);
                        echo $keywordData."\n";
